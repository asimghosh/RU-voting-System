...
<body>
	...
	<div class="container">
		...
		<a class="brand">
		<img src="admin/images/tms.jpg" width="60" height="60">
 	</a>
	<a class="brand">
	 <h2>TMSS Engineering College Voting System</h2>
	 <div class="chmsc_nav"><font size="4" color="white">TMSS Engineering College</font></div>
 	</a>
...
//governor
$result=mysqli_query($link,"select * from candidate where CandidateID='$governor'")or die(mysqli_query());
$row=mysqli_fetch_array($result);
$name=$row['FirstName']."  ".$row['LastName'];
//
//vice-governor
$vice=mysqli_query($link,"select * from candidate where CandidateID='$vice1'")or die(mysqli_query());
$row_vice=mysqli_fetch_array($vice);

$name1=$row_vice['FirstName']."  ".$row_vice['LastName'];
//
//Representative1
$Representative1=mysqli_query($link,"select * from candidate where CandidateID='$representative1'")or die(mysqli_query());
$Representative1_row=mysqli_fetch_array($Representative1);

$name2=$Representative1_row['FirstName']."  ".$Representative1_row['LastName'];
//
//Representative2
$Representative2=mysqli_query($link,"select * from candidate where CandidateID='$representative2'")or die(mysqli_query());
...
//Representative3
$Representative3=mysqli_query($link,"select * from candidate where CandidateID='$representative3'")or die(mysqli_query());
...
if (isset($_POST['vote'])){
$gov=$_POST['gov'];
mysqli_query($link,"insert into votes (CandidateID)values('$gov')")or die(mysqli_error());

$vice=$_POST['vice'];
mysqli_query($link,"insert into votes (CandidateID)values('$vice')")or die(mysqli_error());

$rep1=$_POST['rep1'];
mysqli_query($link,"insert into votes (CandidateID)values('$rep1')")or die(mysqli_error());


$rep2=$_POST['rep2'];
mysqli_query($link,"insert into votes (CandidateID)values('$rep2')")or die(mysqli_error());

$rep3=$_POST['rep3'];
mysqli_query($link,"insert into votes (CandidateID)values('$rep3')")or die(mysqli_error());


mysqli_query($link,"update voters set Status='Voted' where VoterID='$session_id'") or die(mysqli_error());
...
<?php
	...
	$(document).ready(function()
		{
		 var delay = 10000;
		setTimeout(function(){ window.location = 'thankyou.php';}, delay);  
    	});
